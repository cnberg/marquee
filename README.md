...
![智能推荐结果：输入"周星驰的经典电影"，返回带理由的片单](screenshot.png)

哪怕是相当抽象的描述（"主角骑自行车的电影"），语义召回也能找到《偷自行车的人》、
《十七岁的单车》这类主题相关的片子，并把库外的相关电影并列展示出来——所有搜索可
以在「搜索历史」里随时回看，owner 主动 opt-in 后还能生成公开链接分享给朋友。

![搜索历史详情：可回看每一次搜索的完整结果，可分享](search-history.png)
